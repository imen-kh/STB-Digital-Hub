...
export class CropperPositionService {
  resetCropperPosition(
    sourceImage: ElementRef | undefined,
  ...
  resize(event: Event, moveStart: MoveStart, cropperPosition: CropperPosition, maxSize: Dimensions, settings: CropperSettings): void {
      ...
      case 'center':
        {
          const newWidth = Math.min(
            Math.max(settings.cropperScaledMinWidth, Math.abs(moveStart.x2 - moveStart.x1)),
            settings.cropperScaledMaxWidth
          );
          const newHeight = Math.min(
            Math.max(settings.cropperScaledMinHeight, Math.abs(moveStart.y2 - moveStart.y1)),
